...
@pytest.mark.parametrize("program", ["transform", "scan-macro", "new-config", "clear-config", "new-user", "save-config", "load-config", "move-scan", "clear-scan"])
def test_program(program):
    ...
    assert result.returncode == 0, f"Program {program_name} failed with return code {result.returncode}."


def test_macro_programs():
    ...
    for dir in scan_dir.glob("*_om-scan*"):
        if dir.is_dir():
            for filename in macrotext_list_om:
                assert (dir / filename).is_file(), "Failed to copy files from macro run using 'move-scan'." 
...
if __name__ == "__main__":
    programs = ["transform", "scan-macro", "new-config", "clear-config", "new-user", "save-config", "load-config", "move-scan", "clear-scan"]
